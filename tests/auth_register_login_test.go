...
func TestRegisterLogin_Login_HappyPath(t *testing.T) {
	...
	respReg, err := st.AuthClient.Register(ctx, &sso_v1.RegisterRequest{
		Email:    email,
		Password: pass,
	})
	...
	respLogin, err := st.AuthClient.Login(ctx, &sso_v1.LoginRequest{
		Email:    email,
		Password: pass,
		AppId:    appID,
	})
...
func TestRegister_FailCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := st.AuthClient.Register(ctx, &sso_v1.RegisterRequest{
				Email:    tt.email,
...
func TestLogin_FailCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = st.AuthClient.Login(ctx, &sso_v1.LoginRequest{
				Email:    tt.email,
				Password: tt.password,
				AppId:    tt.appID,
			})
